...
RUN apt-get update && apt-get install -y \
  # Install pip so that we can use pip to install yt-dlp (youtube-dl) and streamlink at runtime
  python3-pip \
  # yt-dlp (youtube-dl) needs ffmpeg, but it can't be installed with pip
  ffmpeg \
